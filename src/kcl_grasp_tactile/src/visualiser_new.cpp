...
#include <math.h>
#include <cstdio>
#include <kcl_grasp_msgs/KCL_ContactStateStamped.h>
#include <ros/ros.h>
...
//	glColor3f(0.0f, 1.0f, 1.0f); //Change when necessary
	int i, j;
...
//	glRotatef(120,0,0,1);
	...
	/*else */
	theta1=180-(180/M_PI)*atan2(fz,fx);
	theta2=(180/M_PI)*atan2(fy, sqrt(fx*fx+fz*fz));
	glRotated( 90, 0, 1,0);
	glRotated(theta1, 0,1 , 0);
	glRotated(theta2 ,1, 0, 0);
	//printf("theta1=%f\ntheta2=%f",(180/M_PI)*atan2(fz,fx),(180/M_PI)*atan2( fy,sqrt(fx*fx+fz*fz))); 		// Used for display
	if(Ftot>0.2) 
	{
...
void Callback_contactstate(const kcl_grasp_msgs::KCL_ContactStateStamped::ConstPtr& msg)
{
	/** Scale is meter
	//	Contact location (x, y, z)
...
int main(int argc, char *argv[])
{
	...
	ros::NodeHandle n;
	ros::Subscriber sub = n.subscribe<kcl_grasp_msgs::KCL_ContactStateStamped>("ContactState", 1000, Callback_contactstate);
	...
	/**A = 8.5;	B = 16;	C = 9.5;	Z0 = 0;
	//	Different finger
	A = 16.7;	B = 16.7;	C = 11.5;		Z0 = 0;*/
	A = 17.88;	B = 17.88;	C = 15.5;	Z0 = 0;			//For  the contact finger
